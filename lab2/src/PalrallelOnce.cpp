...
#pragma omp single
    g_normVal = 0;
#pragma omp for reduction(+:g_normVal)
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream csv("../src/testerPy/PerfOnce.csv", std::ios::out);
...
//              std::cout << (NormOf(intermediate.data(), N) / vecBNorm) << std::endl; // debug stuff
        ...
        std::ofstream output("../src/testerPy/OutvecFromOnce.bin", std::ios::binary);
